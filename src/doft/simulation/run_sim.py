...
from doft.utils.utils import estimate_memory_gb

def main():
    """
    Main orchestrator for the DOFT Phase 1 counter-trial.
    Final version with a stable numerical core.
    """
    ...
    gamma = 0.05
    grid_size = 1024 # Example of a reasonably large grid
    ...
    for (a_val, tau_val) in simulation_points:
        for seed in seeds:
            ...
            # The model is now stable and memory-efficient
            model = DOFTModel(
                grid_size=grid_size,
                a=a_val,
                tau=tau_val,
    ...
    meta_data = {
        ...
        'numerical_params': {
            'integrator': 'IMEX (Semi-Implicit Euler)',
            'memory_model': 'Prony (Auxiliary ODEs)',
            'nondimensional_dt': 0.02,
            'a_ref': a_ref,
            'tau_ref': tau_ref
        }
    }
